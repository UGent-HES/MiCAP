...
void micap_reconfigure(u8 newtruthtables[NUMBER_OF_TLUTS_PER_INSTANCE][LUT_CONFIG_WIDTH], const lutlocation location[] ) {
	...
	for(i =0;i<NUMBER_OF_TLUTS_PER_INSTANCE;i++) {
		...
		Status = MiCAP_SetClbBits(RowNum, ColNum, XHI_CLB_LUT_replacement.CONTENTS[location[i].sliceType][location[i].lutType], newtruthtables[i], LUT_CONFIG_WIDTH);

		if (Status != XST_SUCCESS) {
			print("MiCAP_SetClbBits - Fail !\n\r");
		}
	}
...
int MiCAP_SetClbBits(long slice_row, long slice_col, const u8 Resource[][2], const u8 Value[], long NumBits){
	   u8 bottom_ntop;
	    int Status;
	    int clock_row;
...
int MiCAP_DeviceReadFrames(u32 frameAddress, u8 NumFrames, u32 *FrameBuffer)
{
	...
	u32 buff_read[507];
	u32 dummy;
	//frame_address = XHwIcap_Custom_SetupFar7series(Top, Block, HClkRow,  MajorFrame, MinorFrame);
	u32 CTRL_REG = 0x0;
